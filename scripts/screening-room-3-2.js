...
var YOUTUBE_SEARCH_URL = "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&channelId=UC4-hGd01KPF1i9xSWtox0JQ&maxResults=DEFAULT_PAGE_SIZE&q=SEARCH_TOKEN&key=AIzaSyDien4wrgW5R-Ewzq2hRxvoofMgTLVlFO8&alt=json";
var BRIGHTCOVE_SEARCH_URL = "http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=" + DEFAULT_PAGE_SIZE + "&page_number=SEARCH_PAGE_NUMBER_TOKEN&all=tag:screening%20room&all=search_text:" + SEARCH_TOKEN + "&callback=video";
...
//Brightcove vars (global to avoid race conditions)
var player;
var modVP;
var modExp;
var modCon;
...
$(document).ready(function() {	
	...
	**/	
	VideoController.prototype.populateVideoGroupObjectWithVideos = function (response, textStatus, jqXHR) {
		...
		if (typeof jqXHR.video_group_index == 'undefined') {
			buildVideoGroupArrays(jqXHR.is_landing_page, jqXHR.is_search);
		}
		//if this VideoGroup object DOES exist
			...
			$.each(response.items, function (i, item) {
				...
				if (videoController instanceof BrightcoveVideoController)
					...
					this_video = new Video(item.id.videoId, videoController.truncate(item.snippet.title, 50), videoController.truncate(item.snippet.description, 30), "" + YOUTUBE_PLAY_URL + item.id.videoId, item.snippet.thumbnails.medium.url);
	...
	**/	
	VideoController.prototype.handleMoreClick  = function handleMoreClick (event, ui, the_json_datatype){
		...
		//if that PageOfVideos array is NOT already in the array, do ajax call to get the appropriate video array
			...
			var the_ajax_url;
			if (is_search){
				the_ajax_url = this.getSearchAjaxURL(search_text, the_video_group_index, DEFAULT_PAGE_SIZE, the_next_page_token);
			}
			else {
				the_ajax_url = this.getAjaxURL(the_video_group_index,the_next_page_token,DEFAULT_PAGE_SIZE);
			}	
	...
	**/	
	VideoController.prototype.layoutVideos = function (is_landing_page, video_group_index, this_search_page_number, is_search) {
		...
		if (is_landing_page) {
			for (i = 0; i < (DEFAULT_PAGE_SIZE/DEFAULT_HOME_PAGE_COLUMN_SIZE); i++) {
	...
	**/	
	VideoController.prototype.AddVideoGroupToArray = function AddVideoGroupToArray(the_search_text, the_video_group_index, the_json_datatype) { 
		var layout_videos = true;
		var the_ajax_url = this.getSearchAjaxURL(the_search_text, the_video_group_index, DEFAULT_PAGE_SIZE);
	...
	**/	
	VideoController.prototype.buildVideoGroupArrays = function buildVideoGroupArrays(is_landing_page, the_json_datatype) { 
		...
		for (c = 0; c < this.video_groups.length; c++) {
			...
			var the_ajax_url = this.getAjaxURL(c,0,DEFAULT_PAGE_SIZE);
			$.ajax({
				type: "POST",
				//set jsponp dataType value for YouTube API
				dataType: the_json_datatype,
	...
	**/	
	function YouTubeVideoController() { 
		...
		**/	
    	this.video_groups = [
			new VideoGroup("https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&channelId=UC4-hGd01KPF1i9xSWtox0JQ&maxResults=DEFAULT_PAGE_SIZE&q=mad%20men&key=AIzaSyDien4wrgW5R-Ewzq2hRxvoofMgTLVlFO8&alt=json","Mad Men", "Mad Men", "Jon Hamm Highlights"),
			new VideoGroup("https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&channelId=UC4-hGd01KPF1i9xSWtox0JQ&maxResults=DEFAULT_PAGE_SIZE&q=community&key=AIzaSyDien4wrgW5R-Ewzq2hRxvoofMgTLVlFO8&alt=json","Community", "Community", "Community Highlights"), 
			new VideoGroup("https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&channelId=UC4-hGd01KPF1i9xSWtox0JQ&maxResults=DEFAULT_PAGE_SIZE&q=paleyfest&key=AIzaSyDien4wrgW5R-Ewzq2hRxvoofMgTLVlFO8&alt=json","PaleyFest", "PaleyFest", "PaleyFest Highlights"), 
		];	
		...
		**/	
		this.getSearchAjaxURL = function getSearchAjaxURL(the_search_term, the_group_index, the_default_page_size, the_search_page_number) {
			var new_search_url;
			new_search_url = videoController.video_groups[the_group_index].json_call.replace(SEARCH_TOKEN, the_search_term);
			new_search_url = new_search_url.replace("DEFAULT_PAGE_SIZE", the_default_page_size);
	...
	**/	
	function BrightcoveVideoController() { 
		...
		**/	
		...
		this.ajax_feature_video_prepopulate = "http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?find_video_by_id=true&video_id=VIDEO_ID_TOKEN&callback=video";
		...
		**/	
		this.video_groups = [
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:popular&all=tag:screening%20room&sort_by=PLAYS_TOTAL:DESC&callback=video","", "Popular", "Most Watched on paleycenter.org"),
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&none=tag:paleyfest&all=tag:screening%20room&sort_by=CREATION_DATE:DESC&callback=video","", "Recently Added", "Videos Added Every Week"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:paleyfest&all=tag:screening%20room&callback=video","PaleyFest", "PaleyFest", "See Your Favorite Shows & Stars"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:comedy&all=tag:screening%20room&callback=video","Comedy", "Comedy", "Comedy Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:drama&all=tag:screening%20room&callback=video","Drama", "Drama", "Drama Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:music&all=tag:screening%20room&callback=video","Music", "Music", "Music Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:issues&all=tag:screening%20room&callback=video","Issues", "Issues", "Issues Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:scifi&all=tag:screening%20room&callback=video","SciFi", "SciFi", "SciFi Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:paleydocevents&all=tag:screening%20room&callback=video","PaleyDocScene", "PaleyDocScene", "Documentary Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:paleyfest&all=tag:screening%20room&callback=video","PaleyFest", "PaleyFest", "See Your Favorite Shows & Stars"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:paleyafterdark&all=tag:screening%20room&callback=video","PaleyAfterDark/Paley100", "PaleyAfterDark/Paley100", "Patron Membership Events"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:womenatpaley&all=tag:screening%20room&callback=video","Women@Paley", "Women@Paley", "Accomplished women share their talents and insight"), 
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:season%201&all=tag:At%20the%20Paley%20Center&all=tag:screening%20room&callback=video","At the Paley Center Season 1", "At the Paley Center Season 1", "Paley Center TV Series"),
			...
			new VideoGroup("http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=DEFAULT_PAGE_SIZE&page_number=0&all=tag:media_council&all=tag:screening%20room&callback=video","Paley Center Media Council", "Paley Center Media Council", "Meet the Industry Leaders")
		];
		...
		**/	
		this.getSearchAjaxURL = function getSearchAjaxURL(the_search_term, the_group_index, the_default_page_size, the_search_page_number) {
			...
			var new_search_url = videoController.video_groups[the_group_index].json_call.replace(SEARCH_TOKEN, the_search_term);
	...
	**/	
	YouTubeVideoController.prototype.handleSearchClick = function(event, ui, the_ajax_url){
	  return VideoController.prototype.handleSearchClick.call(this, event, ui, YOUTUBE_SEARCH_URL);
	}
	...
	**/	
	BrightcoveVideoController.prototype.handleSearchClick = function(event, ui, the_ajax_url){
	  return VideoController.prototype.handleSearchClick.call(this, event, ui, BRIGHTCOVE_SEARCH_URL);
	}
	...
	**/	
	$("#search_videos").on( "click", function( event, ui ) { 
		event.preventDefault();
		videoController.handleSearchClick (event, ui, YOUTUBE_SEARCH_URL);
	});	
...
* (outside of ready function to avoid race conditions)
* 
**/
...
* (outside of ready function to avoid race conditions for Brightcove player)
**/	
