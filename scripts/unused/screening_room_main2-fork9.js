...
var search_ajax_with_token_bc = "http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=" + DEFAULT_PAGE_SIZE + "&page_number=SEARCH_PAGE_NUMBER_TOKEN&all=tag:screening%20room&all=search_text:" + SEARCH_TOKEN + "&callback=video";
var search_ajax_with_token_yt = "http://mediaorigin.paleycenter.org/wp-content/misc/videos/index.php?search=true&page_size=" + DEFAULT_PAGE_SIZE + "&page_number=SEARCH_PAGE_NUMBER_TOKEN&all=tag:screening%20room&all=search_text:" + SEARCH_TOKEN + "&callback=video";
var search_text = "";
...
/*	
	videoController.prototype.handleSearch = function (event, ui) {
			var the_video_group_index;
			$.each(ui, function (i, item) {
			   the_video_group_index = item.find(".get_video_group").attr("id");
			});
...
/*	
	videoController.prototype.addNewVideoGroup = function (response, textStatus, jqXHR) {
	...
	videoController.prototype.createNewVideoGroupObject = function (response, textStatus, jqXHR) {
		var = new_video_group_index_id;
		//check for video_group existence and adding new one if new
		if (typeof jqXHR.video_group_index == 'undefined') {
			buildVideoGroupArrays(is_landing_page) 
			//new_video_group_index_id = videoController.addNewVideoGroup(response, textStatus, jqXHR);
			//populateVideoGroupObjectWithVideos(response, textStatus, jqXHR);
		}
		else {
			populateVideoGroupObjectWithVideos(response, textStatus, jqXHR);
		}
...
//<iframe style="position:relative;left:188px;top:131px;" id="ytplayer" type="text/html" width="493" height="307" src="https://www.youtube.com/embed/wn0q5XJqu6E" frameborder="0" allowfullscreen=""></iframe>			
		...
		*/
		...
		this.handleSearchClick  = function handleSearchClick (event, ui){
			...
			if (typeof the_video_group_index == 'undefined' || typeof the_search_page_number == 'undefined') {
				//make new video_group object
				alert("search video_group DOES NOT exist as object");
				this.video_groups.push(new video_group("https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&channelId=UC4-hGd01KPF1i9xSWtox0JQ&maxResults=DEFAULT_PAGE_SIZE&q=SEARCH_TOKEN&key=AIzaSyDien4wrgW5R-Ewzq2hRxvoofMgTLVlFO8&alt=json","", "Search Results for: " + search_text, ""));
				this.buildVideoGroupArrays(false, true, search_text);
			}
			//if search video_group object already exists
			else {
				alert("search video_group exists as object");
				var layout_videos = true;
				var is_landing_page = false;
				var is_search = true;
...
*/				
		...
		this.buildVideoGroupArrays = function buildVideoGroupArrays(is_landing_page, is_search, the_search_text) { 
			//var the_page_size = is_landing_page ? 5 : DEFAULT_PAGE_SIZE;
			//alert("the_page_size: " + the_page_size);
			var the_ajax_url;
			alert("Before initializing: is_search: " + is_search);
			var is_search = (typeof is_search == 'undefined') ? false : true;
			alert("After initializing: is_search: " + is_search);
			...
			if (is_search) {
				...
				//build search url
				//the_ajax_url = getAjaxURL3(c,0,DEFAULT_PAGE_SIZE);
				the_ajax_url = getSearchAjaxURLyt(the_search_text);
				layout_videos = true;
	...
	**/		
	...
	function getSearchAjaxURLyt(the_search_term, the_search_page_number) {
		var new_search_url = search_ajax_with_token_yt.replace(SEARCH_TOKEN, the_search_term);
		if (typeof the_search_page_number != 'undefined')
			alert("need to replace SEARCH_NUMBER_TOKEN next page for yt");
